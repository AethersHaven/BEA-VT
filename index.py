...
class PageGenerator:
    ...
    def Initialize(this):
        ...
        for iconFile in iconFiles:
            this.Icons.append(Image.open("images\\"+iconFile).convert("RGBA").resize((this.IconSize, this.IconSize)))
    ...
    def GenerateAndSavePages(this):
        # Clear out old pages
        for fileName in os.listdir(os.getcwd() + "\\exports\\"):
            if fileName.startswith("export-") and fileName.endswith(".jpg"):
                os.remove(os.getcwd() + "\\exports\\" + fileName)

        # Generate and save new pages
        for pageIndex in range(this.PageCount):
            PageGenerator.GeneratePage().save(os.getcwd()+"\exports\export-"+str(pageIndex)+".jpg", quality=300)
